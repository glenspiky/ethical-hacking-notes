## HTTP Status Codes – Practical Notes
...
> 100 Continue
> -this indecates that the client should continue the request or ignore the request if the response is already finished
> 101 Informational responses
...
> 201 Created
> -The request succeeded, and a new resource was created as a result. This is typically the response sent after *POST*requests, or some _PUT_ requests.
...
![Juice shop](./screenshots/image.png)

Canged the basket id and it gave me a 200 ok

![alt text](./screenshots/image-1.png)

Changed a GET to a POST and it gave me a 400 bad request

![alt text](./screenshots/image-2.pngimage.png)
...
There is a global auth check before busines logic
...
Auth middleware -> Role middleware -> Controler
...
401 (no token) → 200 (user token)

### Backend inference:

Authorization / role check missing
...
Change basket ID to another user’s basket
...
Read or manipulate other users’ baskets
...
Changed basket ID from own basket to another user's basket
...
Any authenticated user can access other users’ baskets
